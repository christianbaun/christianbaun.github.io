...
(function($, undefined) {
	...
	// fixUrl by Laza :: adding full path, removing '../'s in local
	
	var fixUrl = function( u ) {
		if ( location.protocol.indexOf('file:') === 0 ) {
			var i, j;
			u = location.href.substring(0, location.href.lastIndexOf('/') + 1) + u;
			while ( (i = u.indexOf('../')) > 0) {
				if ( i === 1 || (j = u.lastIndexOf('/', i - 2)) === -1 ) {
					return u.substring(i + 3);
				}
				u = u.substring(0, j) + u.substring(i + 2);
			}
		}
		return u;
	};
		...
		_init: function() {
			...
			if(this.flash.used) {
				...
				if($.jPlayer.browser.msie && Number($.jPlayer.browser.version) <= 8) {
					...
					var paramStr = [
						'<param name="movie" value="' + fixUrl(this.internal.flash.swf) + '" />',
					...
					htmlObj.setAttribute("data", fixUrl(this.internal.flash.swf));
